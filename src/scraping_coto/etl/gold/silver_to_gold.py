...
# from pyspark.sql.functions import col, explode, row_number, expr, struct, lit, coalesce
...
#     ):
#     """
#     Constructs the full table name for the gold items table.

#     Args:
#         source_catalog_name (str): Source catalog name.
#         source_schema_name (str): Source schema name.
#         source_table_name (str): Source table name.
#         target_catalog_name (str): Target catalog name.
#         target_schema_name (str): Target schema name.
#         target_table_name (str): Target table name.
#         options (dict, optional): Additional options for processing. Defaults to an empty dictionary. Valid options are:
#             - partition_by (list): List of columns to partition the target table by.
#             - mode (str): Write mode for the target table. ['append', 'overwrite', 'ignore', 'errorifexists']
#             - merge_keys (list): List of keys to use for merging data in case of upsert operations.
#             - start_date (str): Date filter to apply when processing data.
#             - end_date (str): End date filter to apply when processing data.
#             - date_column (str): Date column to use for filtering data.
#     """

#     logger.info(f"Loading gold items table: {target_catalog_name}.{target_schema_name}.{target_table_name} from source: {source_catalog_name}.{source_schema_name}.{source_table_name}")
#     logger.info(f"With options: {options}")

#     # Configs
#     target_exists = spark.catalog.tableExists(f'{target_catalog_name}.{target_schema_name}.{target_table_name}')
#     partition_by = options.get('partition_by', None)
#     mode = options.get('mode', 'overwrite')
#     merge_keys = options.get('merge_keys', None)
#     start_date = options.get('start_date', None)
#     end_date = options.get('end_date', None) if options.get('end_date', None) else date.today().isoformat()
#     date_column = options.get('date_column', 'dt')

#     df_dept = (
#         spark.table(f'{source_catalog_name}.{source_schema_name}.departments')
#         .where(f"{date_column} BETWEEN '{start_date}' AND '{end_date}'" if start_date else f"{date_column} <= '{end_date}'")
#         .withColumn(
#             'rn',
#             row_number().over(Window.partitionBy('department_id').orderBy(col(date_column).desc_nulls_last()))
#         )
#         .where('rn = 1')
#         .withColumn(
#             'categories',
#             explode('departments.subCategories')
#         )
#         .withColumn(
#             'subcategories',
#             explode('categories.subCategories2')
#         )
#         .selectExpr(
#             'department_id',
#             'department_name',
...
#                         coalesce(col("ai_generated.product_unit_of_measure"), col("product_quantity.unit"))
#                     ).alias("quantity")
#                 ).alias("actual"),
#                 struct(
...
#     if target_exists and mode == 'merge' and merge_keys:
#         logger.info(f"Merging data into existing table {target_catalog_name}.{target_schema_name}.{target_table_name} using keys: {merge_keys}")

#         delta_table = DeltaTable.forName(spark, f'{target_catalog_name}.{target_schema_name}.{target_table_name}')
#         (
#             delta_table
#             .alias("target")
#             .merge(
#                 df_gold_items.alias("source"),
...
#         logger.info(f"Appending data to existing table {target_catalog_name}.{target_schema_name}.{target_table_name} and replacing partitions if duplicates exist")
...
#             .alias("t")
#             .merge(
#                 df_gold_items.alias("s"),
#                 f"t.{date_column} = s.{date_column}" 
#             ).whenMatchedUpdateAll()
#             .whenNotMatchedInsertAll()
#             .execute()
#         )
    
#     elif not target_exists or mode == 'overwrite':
#         logger.info(f"Writing data to table {target_catalog_name}.{target_schema_name}.{target_table_name} with mode {mode}")
#         writer = (
#             df_gold_items
#             .write
#             .mode(mode)
...
#         writer.saveAsTable(f'{target_catalog_name}.{target_schema_name}.{target_table_name}')
